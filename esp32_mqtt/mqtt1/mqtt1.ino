...
const char* mqtt_server = "192.168.77.26";
...
void blinkLed3() {
  static int blinkOffCount = 0;       // Đếm số lần LED ở trạng thái "off"
  static unsigned long startCycleTime = 0; // Thời điểm bắt đầu chu kỳ
  unsigned long now = millis();

  // Nếu đang trong chu kỳ mới, đặt lại bộ đếm và trạng thái LED
  if (now - startCycleTime >= 10000) { // Chu kỳ 10 giây
    startCycleTime = now;  // Bắt đầu chu kỳ mới
    blinkOffCount = 0;     // Reset bộ đếm
    digitalWrite(led3, LOW); // Đảm bảo LED tắt
    led3State = false;
  }

  // Chỉ nhấp nháy nếu chưa đạt đủ 3 lần "off"
  if (blinkOffCount < 3 && now - lastBlinkTime > 500) {  // Nhấp nháy với chu kỳ 0.5 giây
    led3State = !led3State;
    digitalWrite(led3, led3State);
    lastBlinkTime = now;

    // Gửi trạng thái LED qua MQTT khi thay đổi
    if (led3State) {
      sendLed3Status("on");
    } else {
      sendLed3Status("off");
      blinkOffCount++; // Tăng số lần khi LED chuyển sang "off"
    }
  }
}
